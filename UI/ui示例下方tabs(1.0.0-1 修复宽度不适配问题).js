    ...
    return (
      ...
      >
        <img
          w="{{tabs_data.srcSize}}"
          id="_src"
          tint="{{tabs_data.selectColor.off}}"
        />
        <text
          w="auto"
          id="_text"
          textSize="{{tabs_data.textSize}}"
          textColor="{{tabs_data.selectColor.off}}"
        />
      </vertical>
    );
  };
  ...
  function Tabs_layout() {
    ...
    this.defineAttr("data", (view, attr, value, defineSetter) => {
      ...
      for (var i = 0; i < tabs_data.data.length; i++) {
        ...
        ui.inflate(
          <tabs_btn-layout data="{{time}}" layout_weight="1" />,
          view._tabs,
          true,
        );
      }
      ...
      tabs_data.tabs_h
        ? (_color = tabs_data.selectColor.on)
        : (_color = "#00000000");
...
ui.viewpager.setOnPageChangeListener({
  ...
  onPageSelected: function (index) {
    ...
    tabs_view[selectView]._text.setTextColor(
      colors.parseColor(tabs_data.selectColor.off),
    );
    //设置当前页面 图案和字体颜色为选中颜色 tabs_data.selectColor.on
    tabs_view[index]._src.attr("tint", tabs_data.selectColor.on);
    tabs_view[index]._text.setTextColor(
      colors.parseColor(tabs_data.selectColor.on),
    );
    //更改标题 title 内容
