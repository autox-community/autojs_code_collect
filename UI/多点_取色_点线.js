      ...
      case 1:
        ...
        let path = selectFile("/sdcard/", "", function (name) {
          return (
            name.endsWith(".jpg") ||
            name.endsWith(".png") ||
            files.isDir(files.join("/sdcard", name))
          );
        });
        if (path) {
          var IMG = images.read(path);
...
ui.clear2_ary.click(function () {
  if (canvasAD.PointsAry.strokeAry.length) {
    threads.start(function () {
      ...
      if (sum) {
        sum =
          sum <= canvasAD.PointsAry.strokeAry.length
            ? sum
            : canvasAD.PointsAry.strokeAry.length;
        canvasAD.PointsAry.strokeAry.splice(
          canvasAD.PointsAry.strokeAry.length - sum,
          sum,
        );
      }
    });
        ...
        default:
          ...
          switch (event.getActionMasked()) {
            ...
            case event.ACTION_UP:
              ...
              this.PointsAry.strokeAry = this.PointsAry.strokeAry.concat(
                this.PointsAry.strokePoints,
              );
              this.PointsAry.strokePoints = new Array();
  ...
  this.addPoint2 = function (strokePoints, X, Y) {
    ...
    if (this.imgRect.contains(x, y)) {
      ...
      if (strokePoints.length >= 1) {
        ...
        if (
          !this.PointsAry.strokeAry.concat(strokePoints).some(function (ary) {
