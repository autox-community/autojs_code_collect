...
function OrientationListener() {
  ...
  this.javaListener = new JavaAdapter(
...
var ctss = threads.start(function () {
  sensors
    .register("linear_acceleration", sensors.delay.ui)
...
function hx() {
  ...
  var fh = threads.start(function () {
    if ((upx > screenmid2 + 300 || upx < screenmid2 - 300) && upy < wp - wcha) {
      if (currentPackage() == "com.blackberry.hub") {
      } else {
        launch("com.blackberry.hub");
        toastLog("启动Hub");
      }
    } else if (
      (upx < screenmid2 - 200 || upx > screenmid2 + 200) &&
      upy > wp - wcha &&
      upy < wp - 100
    ) {
      back();
...
function qnscreen() {
  app.startActivity({
    ...
    className:
      "com.forfan.bigbang.component.activity.screen.ScreenCaptureActivity",
  });
}
