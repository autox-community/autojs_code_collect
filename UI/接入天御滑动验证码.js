...
function gg(stringExtra) {
  threads.start(function () {
    ...
    window.text.setOnTouchListener(function (view, event) {
      switch (event.getAction()) {
        ...
        case event.ACTION_MOVE:
          ...
          if (
            Math.abs(event.getRawY() - y) > 15 ||
            Math.abs(event.getRawX() - x) > 15
          )
            is_wrap = true;
        ...
        case event.ACTION_UP:
          //手指弹起时如果偏移很小则判断为点击
          if (
            Math.abs(event.getRawY() - y) < 6 &&
            Math.abs(event.getRawX() - x) < 6
          ) {
            if (window.text.getParent().getChildCount() == 1) ck();
    ...
    var ck = function () {
      ...
      var webCC = new JavaAdapter(WebChromeClient, {
        onJsPrompt: function (webView, str, str2, str3, jsPromptResult) {
          if (str2.indexOf("state") != -1 && str2.indexOf("info") != -1)
            return false;
    ...
    function getContent(str) {
      var sContent1 =
        '<html><head lang="zh-CN"><title>验证码</title><meta charset="UTF-8"><meta name="renderer" content="webkit"><meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no"><meta name="format-detection" content="address=no; email=no"></head><body></body><script src="';
      var sContent3 =
