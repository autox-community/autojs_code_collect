...
var scrollView = XmlToView(<scroll id="vt_0"></scroll>);
...
function getMyView(path) {
  ...
  this.view = XmlToView(
    <vertical id="vt_1" margin="5">
      ...
      <vertical id="vt_2" marginLeft="25"></vertical>
    </vertical>,
  );
  ...
  this.view.name.click(() => {
    if (this.file.file.isDirectory()) {
      ...
      if (this.thread ? !this.thread.isAlive() : true) {
        //线程没有运行。
        if (!this.file.isOpen) {
          this.isadd = true;
...
function myFile(path) {
  ...
  if (this.file.isDirectory()) {
    this.list = function () {
      return this.file.listFiles().map((file) => {
