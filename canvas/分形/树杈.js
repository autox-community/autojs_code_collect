...
ui.board.on("draw", function (canvas) {
  ...
  i += 0.3;
  n = 9;
  koch(canvas, paint, 0, 0, 0, -150, Math.sin(i) * 3 + 20, n);
});
...
function koch(canvas, paint, x0, y0, x1, y1, a, k) {
  ...
  x2 =
    l *
      ((x1 - x0) * Math.cos((a * Math.PI) / 180) -
        (y1 - y0) * Math.sin((a * Math.PI) / 180)) +
    x1; /*由上面的运算可以得到其余三点 坐标的计算式*/
  y2 =
    l *
      ((x1 - x0) * Math.sin((a * Math.PI) / 180) +
        (y1 - y0) * Math.cos((a * Math.PI) / 180)) +
    y1;
  x3 =
    l *
      ((x1 - x0) * Math.cos((-a * Math.PI) / 180) -
  ...
  y3 =
    ...
    y1;
  if (k > 1) {
    /*如果迭代次数大于1，就继续迭代下去，即执行以下程序*/ koch(
      canvas,
      paint,
      x1,
      y1,
