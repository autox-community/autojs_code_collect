...
ui.but_s.click(function () {
  setClip(
    "var gesturesAry=" +
      JSON.stringify(MainGesturesAry) +
      ";\nfor(let i=0;i<gesturesAry.length;i++){gestures.apply(null, gesturesAry[i]);sleep(" +
      Tss +
      ");}",
  );
  toast("已复制");
...
ui.but_b.click(function () {
  threads.start(function () {
    var p = dialogs.prompt(
      "保存路径",
      storage.get("savePath", "/sdcard/脚本/LZ动作.js"),
    );
...
ui.canvas.on("draw", (canvas) => {
  ...
  paint.setStyle(Paint.Style.STROKE);
  canvas.drawText(
    String(MainGesturesAry.length + "个动作"),
    0,
    textSize,
    paint,
  );
        ...
        default:
          ...
          switch (event.getActionMasked()) {
            case event.ACTION_DOWN:
              ...
              (vrx = RX - X), (vry = RY - Y);
              kg = true;
              TouchPointRecord.splice(I, 0, [
                0,
                1,
                [Math.floor(X + vrx), Math.floor(Y + vry)],
              ]);
