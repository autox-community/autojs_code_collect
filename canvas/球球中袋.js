...
var BALL = [[0.5, 0.5]];
var BALLColor = [
  colors.argb(255, random(0, 255), random(0, 255), random(0, 255)),
];
var BallWidth = 500;
...
ui.canvas.on("draw", function (canvas) {
  try {
    ...
    if (!gameover) {
      ...
      //界限
      BALL[0][0] =
        BALL[0][0] >= 0 && BALL[0][0] < 1 ? BALL[0][0] : BALL[0][0] < 0 ? 0 : 1;
      BALL[0][1] =
        BALL[0][1] >= 0 && BALL[0][1] < 1 ? BALL[0][1] : BALL[0][1] < 0 ? 0 : 1;
      ...
      for (var i = 0; i < FOOD.length; i++) {
        ...
        if (x < w && x >= 0 && y < h && y >= 0) {
          if (
            (BallWidth * Ratio - weiyi(x - w / 2, y - h / 2)) /
              (FOOD[i][2] * Ratio) >=
            0.7
          ) {
            BallWidth = Math.sqrt(
              BallWidth * BallWidth + FOOD[i][2] * FOOD[i][2],
            );
            FOOD.splice(i, 1);
            var color = colors.argb(
              255,
              random(0, 255),
              random(0, 255),
              random(0, 255),
      ...
      if (Touch.length) {
        for (var i = 0; i < Touch.length; i++) {
          ...
          canvas.drawText(
            i,
            Touch[i].X - 200,
            Touch[i].Y + 0.365 * size,
            paint,
          );
          if (
            i == 1 &&
            BallWidth > Math.sqrt(bigFoodWidth * bigFoodWidth * 2)
          ) {
            var A = kdfx(direction.Y, 0);
            var B = getsdxyz(A.x, A.z, 0, BallWidth * Ratio * 1.6);
            var x = (B.x / Ratio + cx) / Width,
              y = (B.y / Ratio + cy) / Height;
            x = x >= 0 && x < 1 ? x : x < 0 ? 0 : 1;
            y = y >= 0 && y < 1 ? y : y < 0 ? 0 : 1;
            BallWidth = Math.sqrt(
              BallWidth * BallWidth - bigFoodWidth * bigFoodWidth,
            );
          }
          var color = colors.argb(
            255,
...
function random(A, B) {
  return Math.round(
    Math.random() * Math.abs(Math.round(A - B)) + (A < B ? A : B),
  );
}
