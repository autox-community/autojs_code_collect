...
var currentpage = 0;
if (
  storage.get("finished") == false &&
  confirm("检测到未完成的任务，是否继续？")
) {
...
function SavePage(p, s) {
  ...
  if (pins) pins = pins[0].match(/<li.*?\d+/g);
  if (pins) pins = pins.map((i) => i.match(/\d+/));
  else alerte("没有搜索到图集");
  ...
  for (i = currentpin; i < pins.length; i++) {
    ...
    var path =
      files.getSdcardPath() + "/妹子图/" + s + "/" + p + "/" + title + "/";
    files.createWithDirs(path);
    !files.isDir(path) && alert("文件夹创建失败");
    toast(title + "：共" + pages + "张图片");
    //循环保存单图集全部图片
    for (ii = 1; ii <= pages; ii++) {
      var imgurl = loadUrl("http://m.mzitu.com/" + id + "/" + ii).match(
        /<figure>.*\n.*(https?:\/\/i\.meizitu.*(\.[^\"]*))/,
      );
      var ext = imgurl[2];
      imgurl = imgurl[1];
...
function loadUrl(url) {
  ...
  res.statusCode != 200 &&
    alerte("服务器错误: " + res.statusCode + " " + res.statusMessage);
