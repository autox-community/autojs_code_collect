...
function BDTB() {
  ...
  this.getPageNumAndReply = function (page) {
    var pattern = new RegExp(
      '<li class="l_reply_num.*?><span class=".*?>(\\d+)</span>.*?>(\\d+)</span>',
      "gim",
    );
    pattern.compile(pattern);
    result = pattern.exec(page);
  ...
  this.getContent = function (page) {
    var pattern = new RegExp(
      '<img username="([\\s\\S]*?)"[\\s\\S]*?<div id="post_content[\\s\\S]*?>([\\s\\S]*?)</div>',
      "gim",
    );
    pattern.compile(pattern);
    ...
    if (result) {
      ...
      for (let i = 0; i < result.length; i++) {
        var 贴子作者正则 = /<img username="(.*?)"/gim;
        var 贴子内容正则 = /<div id="post_content[\s\S]*?>([\s\S]*?)<\/div>/gim;
        var 贴子 = {
          作者: "",
          内容: "",
        };
        贴子.作者 = result[i].match(贴子作者正则);
        贴子作者正则 = /<img username="(.*?)"/gim;
        贴子.作者 = 贴子作者正则.exec(贴子.作者[0]);
        贴子内容正则 = /<div id="post_content[\s\S]*?>([\s\S]*?)<\/div>/gim;
