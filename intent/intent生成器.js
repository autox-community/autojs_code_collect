...
ui.emitter.on("create_options_menu", (menu) => {
  ...
  mSearch.setShowAsAction(
    mSearch.SHOW_AS_ACTION_COLLAPSE_ACTION_VIEW | mSearch.SHOW_AS_ACTION_ALWAYS,
  );
});
...
var aboutUi = {
  ui: ui.inflate(
    <vertical padding="15dp">
      <text textColor="black" textSize="15sp">
        本生成器用于生成Auto.js可用的启动Intent的代码
      </text>
    </vertical>,
  ),
  activate: function () {
...
function stringifyIntentFlag(flag) {
  var i,
    ...
    keys,
    r = [];
  keys = Object.keys(o).filter(
    (e) => e.startsWith("FLAG_") && !e.startsWith("FLAG_RECEIVER_"),
  );
  for (i in keys) {
    e = o[keys[i]];
...
function intentAsJson(intent) {
  ...
  if (intent.component) {
    r.push(
      "packageName: " + JSON.stringify(String(intent.component.packageName)),
    );
...
function copyIntent(code) {
  dialogs.select(
    "操作",
    ["查看对象代码", "复制为app.intent", "复制为app.startActivity", "直接启动"],
    (i) => {
      switch (i) {
        case 0:
...
var launchUi = {
  ...
  filterList: function (query) {
    ...
    return this.listData.filter(
      (e) => e.name.indexOf(query) >= 0 || e.packageName.indexOf(query) >= 0,
    );
  },
  activate: function () {
    currentUi = this;
    ...
    <vertical gravity="center">
      ...
      <list id="hidden_list" h="*" w="*">
        <vertical padding="8dp" w="*">
          ...
          <text
            textSize="12sp"
            text="{{this.packageName + ' (' + this.intents.length + '个Activity)'}}"
          />
        </vertical>
      </list>
    </vertical>,
        ...
        (i) => {
          ...
          copyIntent(
            "{\n" +
              TAB +
              "action: " +
              JSON.stringify("android.intent.action.VIEW") +
              ", //此处可为其他值\n" +
              TAB +
              "packageName: " +
              JSON.stringify(item.packageName) +
              ",\n" +
              TAB +
              "className: " +
              JSON.stringify(item.intents[i].className) +
              "\n" +
              TAB +
...
var openUi = {
    ...
    <vertical>
      <horizontal gravity="center">
        <text
          ...
          padding="10dp"
        />
        <button id="open_edituri" textSize="14sp" h="*">
          编辑
        </button>
      </horizontal>
    ...
    ui.open_list.on("item_click", (item) => {
      copyIntent(
        "{\n" +
          ...
          TAB +
          "className: " +
          JSON.stringify(item.className) +
          ",\n" +
          TAB +
          "data: " +
          compatUriToSource(this.uri) +
          ",\n" +
          (this.mimeType
            ? TAB + "type: " + JSON.stringify(this.mimeType) + ",\n"
            : "") +
          TAB +
          "flags: " +
          JSON.stringify(
  ...
  updateUI: function () {
    ...
    this.uri = this.uri || "";
    this.mimeType =
      this.mimeType == "*/*" ? this.getMime() || "*/*" : this.mimeType;
    ui.open_uri.setText(
      "URI: " +
        (this.uri
          ? this.uri + (this.mimeType ? " (" + this.mimeType + ")" : "")
          : "(空)"),
    );
        ...
        name:
          (e.labelRes != 0
            ...
            : e.activityInfo.labelRes != 0
              ...
              : plabel) +
          " (" +
          shortenName(e.activityInfo.name) +
          ")",
  ...
  filterList: function (query) {
    ...
    return this.listData.filter(
      (e) =>
        e.name.indexOf(query) >= 0 ||
        e.packageName.indexOf(query) >= 0 ||
        e.className.indexOf(query) >= 0 ||
        e.packageLabel.indexOf(query) >= 0,
    );
  },
  activate: function () {
    currentUi = this;
