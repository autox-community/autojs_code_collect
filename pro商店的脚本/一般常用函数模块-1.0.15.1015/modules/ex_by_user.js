...
module.exports = {
  ...
  enableHttpProxy: function (ip, port) {
    ...
    var n = java.net.Proxy,
      o = java.net.InetSocketAddress,
      p = new Packages.okhttp3.OkHttpClient.Builder().proxy(
        new n(n.Type.HTTP, new o(ip, port)),
      );
  ...
  getListFilePath: function (i) {
    function r(i) {
      ...
      for (var s in e) {
        if ("/" == i.charAt(i.length - 1)) var a = i + e[s];
        else var a = i + "/" + e[s];
    ...
    return java.math
      .BigInteger(
        ...
        java.security.MessageDigest.getInstance("MD5").digest(
          java.lang.String(data).getBytes(),
        ),
      )
      .toString(16);
  },
  ...
  pingTest: function (target, count) {
    var res = shell(
      "ping -c " + count + " " + target + " |grep 'rtt' |awk '{print $4}'",
    ).result;
    res = res.replace("\n", "");
  ...
  Base64: function () {
    ...
    this.decode = function (str) {
      return java.lang.String(
        android.util.Base64.decode(java.lang.String(str).getBytes(), 2),
      );
    };
  },
  /**
  ...
  addContact: function (name, phoneNumber) {
    ...
    try {
      ...
      values.put(
        ...
        new java.lang.Integer(rawContactId),
      );
      values.put(
  ...
  clearContact: function () {
    ...
    while (cursor.moveToNext()) {
      ...
      context
        .getContentResolver()
        .delete(ContactsContract.RawContacts.CONTENT_URI, where, whereparams);
    }
    return true;
  },
};
