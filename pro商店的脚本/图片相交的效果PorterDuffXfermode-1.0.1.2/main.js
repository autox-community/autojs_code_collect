...
var imgPath1 = files.join(sdPath, "01.png");
var imgPath2 = files.join(sdPath, "02.png");
var bitmap01;
var bitmap02;
bitmap01 = getBitmap();
bitmap02 = bitmap01.copy(Bitmap.Config.ARGB_8888, true);
loadUI();

function getBitmap() {
  var secondImgId = context
    .getResources()
    .getIdentifier("ic_android_eat_js", "drawable", context.getPackageName());
  ...
  var myDrawableBitmapWidth = myDrawableBitmap.getWidth();
  var myDrawableBitmapHeight = myDrawableBitmap.getHeight();
  log(myDrawableBitmapWidth, myDrawableBitmapHeight);
  // 得到新的宽高与原宽高的比例
  var scaleWidth = bitmapWidth / myDrawableBitmapWidth;
  var scaleHeight = bitmapHeight / myDrawableBitmapHeight;
  var matrix = new Matrix();
  matrix.postScale(scaleWidth, scaleHeight);
  // 生成新的Bitmap
  var myDrawableBitmap = Bitmap.createBitmap(
    myDrawableBitmap,
    0,
    0,
    myDrawableBitmapWidth,
    myDrawableBitmapHeight,
    matrix,
    false,
  );
  return myDrawableBitmap;
}
...
function loadUI() {
  ui.layout(
    <vertical>
      ...
      <text id="threshold" textSize="30sp">
        seekBarValue=0
      </text>
      <vertical>
  ...
  ui.board.on("draw", function (canvas) {
    ...
    // 设置图片相交的模式
    xfermode = eval(
      "new PorterDuffXfermode(PorterDuff.Mode." + currentXfermode + ")",
    );
    paint.setXfermode(xfermode);
    canvas.drawBitmap(
      bitmap02,
      bitmap01.getWidth() + 100 + imgOffset,
      0,
      paint,
    );
