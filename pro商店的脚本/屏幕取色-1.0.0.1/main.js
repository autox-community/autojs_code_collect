...
var window = floaty.rawWindow(
  <vertical
    id="vertical"
    bg="#aaaaaa"
    w="{{Math.floor(device.width*0.8)}}px"
    h="{{Math.floor(device.width*0.8)}}px"
    gravity="center"
  >
...
function 算坐标(X, Y, data, img) {
  ...
  var y = Y / data.scale;
  x = Math.floor(
    0 <= x && x < img.getWidth() ? x : 0 <= x ? img.getWidth() - 1 : 0,
  );
  y = Math.floor(
    0 <= y && y < img.getHeight() ? y : 0 <= y ? img.getHeight() - 1 : 0,
  );
    ...
    sw: switch (
      ...
      case event.ACTION_MOVE:
        ...
        if (PC == 1) {
          ...
          var Y1 = event.getY(1);
          var SS = weiyi([
            Touch[id1].X - Touch[id].X,
            Touch[id1].Y - Touch[id].Y,
          ]);
  ...
  var a = files
    .listDir(path, function (name) {
      return (
        name.endsWith(".jpg") ||
        name.endsWith(".png") ||
        files.isDir(files.join(path, name))
      );
    })
    .sort();
...
function 悬浮控制(window, windowid, ar) {
  ...
  this.toScreenEdge = (d) => {
    ...
    var sx = window.getX() + window.getWidth() / 2,
      sy = window.getY() + window.getHeight() / 2 + 66;
    var cx =
      sx < this.Width - sx ? -sw : this.Width + sw - window.getWidth() + 72;
    return [
      [x, y],
      [cx, y],
    ];
