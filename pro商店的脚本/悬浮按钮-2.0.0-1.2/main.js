      ...
      dialogs
        ...
        .on("single_choice", (index) => {
          switch (index) {
            ...
            case 1:
              auto.service != null
                ? (setClip(Pack), toast("已复制到剪切板"))
                : toast("无障碍服务未启动");
              break;
            case 2:
              auto.service != null
                ? (setClip(Acti), toast("已复制到剪切板"))
                : toast("无障碍服务未启动");
              break;
            case 3:
              toast("自行添加");
  ...
  butLogoLayout.prototype.render = function () {
    return (
      <card
        ...
        w="{{but_w_h}}"
        h="{{but_w_h}}"
        cardCornerRadius="{{but_w_h}}"
        cardBackgroundColor="#99ffffff"
        cardElevation="0"
...
var w_menu = floaty.rawWindow(
  ...
  >
    //
    <butLogo-layout
      name="{{but_data.menu_1.name}}"
      src="{{but_data.menu_1.src}}"
      bg="{{but_data.menu_1.bg}}"
      layout_gravity="center"
    />
    <butLogo-layout
      name="{{but_data.menu_2.name}}"
      src="{{but_data.menu_2.src}}"
      bg="{{but_data.menu_2.bg}}"
      layout_gravity="center"
    />
    <butLogo-layout
      name="{{but_data.menu_3.name}}"
      src="{{but_data.menu_3.src}}"
      bg="{{but_data.menu_3.bg}}"
      layout_gravity="center"
    />
    <butLogo-layout
      name="{{but_data.menu_4.name}}"
      src="{{but_data.menu_4.src}}"
      bg="{{but_data.menu_4.bg}}"
      layout_gravity="center"
    />
    <butLogo-layout
      name="{{but_data.menu_5.name}}"
      src="{{but_data.menu_5.src}}"
      bg="{{but_data.menu_5.bg}}"
      layout_gravity="center"
    />
  </frame>,
);
...
var w_logo = floaty.rawWindow(
  <butLogo-layout
    id="_but"
    name="{{but_data.logo.name}}"
    src="{{but_data.logo.src}}"
    alpha="0.4"
...
for (let i = 0; i < 2; i++) {
  ...
  for (let j = 0; j < menu_view.length; j++) {
    menu_X[i][j] = parseInt(
      0 + Math.cos(((Math.PI * 2) / 360) * (degree - 90)) * menu_r,
    );
    menu_Y[i][j] = parseInt(
      0 + Math.sin(((Math.PI * 2) / 360) * (degree - 90)) * menu_r,
    );
...
new ContextWrapper(context).registerReceiver(
  (intent_CHANGED = new BroadcastReceiver({
...
var id_time_0 = setInterval(() => {
  if (w_logo._but.getWidth() && !data__) {
    data__ == true;
    but_logo_r = -dp2px(
      parseInt((px2dp(w_logo._but.getWidth()) - but_w + port_x) / 2),
    );
    getScreenDirection();
    setTimeout(() => {
      ui.run(() => {
...
function getScreenDirection() {
  ...
  if (context.getResources().getConfiguration().orientation == 1) {
    ...
    device.width < device.height
      ? (log("竖屏宽高异常自动修复"), (_w = device.width), (_h = device.height))
      : ((_w = device.height), (_h = device.width));
  } else {
    screen_rotation = true;
    device.width > device.height
      ? (log("横屏宽高异常自动修复"), (_w = device.width), (_h = device.height))
      : ((_w = device.height), (_h = device.width));
  }
  but_orientation
    ? (X = _w - dp2px(but_w_h) + dp2px((but_w_h - but_w) / 2))
    : (X = 0 + but_logo_r);
  setTimeout(function () {
    ui.run(() => {
  ...
  setTimeout(function () {
    ...
    if (menu_switch) {
      ...
      for (let i = 0; i < menu_view.length; i++) {
        animationX[i] = ObjectAnimator.ofFloat(
          menu_view[i]._bg,
          "translationX",
          menu_X[e][i],
          0,
        );
        animationY[i] = ObjectAnimator.ofFloat(
          menu_view[i]._bg,
          "translationY",
          menu_Y[e][i],
          0,
...
function animation_port(event) {
  ...
  but_orientation
    ? ((PX = _w - dp2px(but_w_h) + but_logo_r),
      (X = [windowX + (event.getRawX() - x), PX]))
    : ((PX = 0 + but_logo_r), (X = [windowX + (event.getRawX() - x), PX]));
  //动画信息
  w_logo_a._but.attr("visibility", "visible");
  animator = ObjectAnimator.ofFloat(w_logo_a._but, "translationX", X);
  animatorY = ObjectAnimator.ofFloat(
    w_logo_a._but,
    "translationY",
  ...
  switch (event.getAction()) {
    ...
    case event.ACTION_MOVE:
      ...
      if (!yd) {
        ...
        if (
          Math.abs(event.getRawY() - y) > 30 ||
          Math.abs(event.getRawX() - x) > 30
        ) {
          view.attr("alpha", "1");
          yd = true;
        }
    ...
    case event.ACTION_UP:
      ...
      if (
        ...
        //移动弹起  判断要停靠的方向
        windowX + (event.getRawX() - x) < _w / 2
          ? (but_orientation = false)
          : (but_orientation = true);
