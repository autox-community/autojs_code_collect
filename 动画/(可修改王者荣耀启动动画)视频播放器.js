    ...
    <frame w="*" h="*">
      <list id="list" h="*">
        <linear w="*" bg="?selectableItemBackground">
          <img
            src="@drawable/ic_videocam_black_48dp"
            tint="white"
            bg="#ff5722"
            w="50"
            h="50"
...
var 王者动画路经 =
  "/storage/emulated/0/Android/data/com.tencent.tmgp.sgame/files/Resources/Splash_move/";
...
ui.text.click(function (v) {
  threads.start(function () {
    ...
    //toastLog(size);
    if (
      new java.io.File(files.join(王者动画路经, "王者动画.mp4")).exists() &&
      dialogs.confirm("是否还原动画")
    ) {
      toastLog("执行还原操作");
      if (
        files.remove(files.join(王者动画路经, 王者动画名)) &&
        files.rename(files.join(王者动画路经, "王者动画.mp4"), 王者动画名)
      ) {
        toastLog("还原成功");
      }
      return;
    ...
    if (
      dialogs.confirm("是否修改为王者荣耀启动动画") &&
      dialogs.confirm("不清楚是否有封号的风险,作者不负责") &&
      (size >= 120000 ? dialogs.confirm("文件过大不建议") : true)
    ) {
      toastLog("执行修改操作");
      if (
        !new java.io.File(files.join(王者动画路经, "王者动画.mp4")).exists()
        ...
        if (
          !files.rename(files.join(王者动画路经, 王者动画名), "王者动画.mp4")
        ) {
          toastLog("操作失败");
          return;
        }
      ...
      if (
        !files.copy(files.join(dir, name), files.join(王者动画路经, 王者动画名))
      ) {
        toastLog("操作失败");
        return;
      }
...
ui.video.setOnCompletionListener(function () {
  ...
  CurrentPlay++;
  CurrentPlay =
    0 <= CurrentPlay && CurrentPlay < videoFilesList.length
      ? CurrentPlay
      : 0 <= CurrentPlay
        ? 0
        : videoFilesList.length - 1;
  var path = files.join(
    videoFilesList[CurrentPlay].dir,
    videoFilesList[CurrentPlay].name,
  );
  // toastLog(path);
  ui.run(() => {
...
function searchFiles(dir, list, fileExts, C) {
  ...
  files.listDir(dir).forEach((fileName) => {
    ...
    for (var i = 0; i < fileExts.length; i++) {
      //如果文件名的后缀是音乐格式
      if (
        fileName.split(".")[1] == fileExts[i] &&
        fileName.endsWith(fileExts[i])
      ) {
        //则把它添加到列表中
        list.push({
          name: fileName,
