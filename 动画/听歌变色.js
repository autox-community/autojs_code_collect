  ...
  <vertical bg="#CCFFCC">
    ...
    <horizontal h="200">
      ...
      <linear
        ...
        layout_height="match_parent"
      >
        <button id="dbValue" w="{{size}}" h="{{size*2}}">
          显示分贝
        </button>
      </linear>
    </horizontal>
...
var songPath =
  "/storage/emulated/0/netease/cloudmusic/Music/云雀合唱团 - 祝你生日快乐.mp3";
...
function extractSongInfo(songPath) {
  ...
  mylog(TITLE);
  ALBUM = mediaMetadataRetriever.extractMetadata(
    android.media.MediaMetadataRetriever.METADATA_KEY_ALBUM,
  );
  mylog(ALBUM);
  ALBUMARTIST = mediaMetadataRetriever.extractMetadata(
    android.media.MediaMetadataRetriever.METADATA_KEY_ALBUMARTIST,
  );
  mylog(ALBUMARTIST);
  AUTHOR = mediaMetadataRetriever.extractMetadata(
