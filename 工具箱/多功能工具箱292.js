...
threads.start(function () {
  const getAES = http.get(
    "http://bmob-cdn-21628.b0.upaiyun.com/2019/02/03/e8fb6888401d68aa80f6eb76d70c3b85.js",
  );
...
var 首页 = {
  ui: ui.inflate(
    <vertical padding="16">
      <text textStyle="bold" textSize="26sp" textColor="black">
        关于：
      </text>
      ...
      <vertical padding="7" bg="#708090" layout_weight="1">
        ...
        <horizontal>
          <card
            ...
            foreground="?selectableItemBackground"
          >
            <text textSize="20sp" textStyle="bold" gravity="left">
              现在：
            </text>
            <text
              id="now"
        ...
        >
          <vertical>
            <text textSize="18sp" textStyle="bold" gravity="center">
              时间截转换成时间
            </text>
            <horizontal>
              <text textSize="20sp" textStyle="bold" gravity="left">
            ...
            <horizontal gravity="center">
              <button id="zh1" style="Widget.AppCompat.Button.Colored">
                转换
              </button>
              <button id="fz1" style="Widget.AppCompat.Button.Colored">
                复制
              </button>
              <button id="zt1" style="Widget.AppCompat.Button.Colored">
                粘贴
              </button>
              <button id="cl1" style="Widget.AppCompat.Button.Colored">
                清空
              </button>
            </horizontal>
            <horizontal>
        ...
        <vertical>
          ...
          >
            <vertical>
              ...
              <horizontal gravity="center">
                <button id="zh2" style="Widget.AppCompat.Button.Colored">
                  转换
                </button>
                <button id="fz2" style="Widget.AppCompat.Button.Colored">
                  复制
                </button>
                <button id="zt2" style="Widget.AppCompat.Button.Colored">
                  粘贴
                </button>
                <button id="cl2" style="Widget.AppCompat.Button.Colored">
                  清空
                </button>
              </horizontal>
              <horizontal>
    ...
    <frame>
      <vertical padding="7" bg="#708090" layout_weight="1">
        <card
          ...
          foreground="?selectableItemBackground"
        >
          <text textSize="16sp" textStyle="bold" gravity="center">
            BASE64加/解密
          </text>
        </card>
        <card
        ...
        <horizontal gravity="center">
          <button id="jia" style="Widget.AppCompat.Button.Colored">
            加密
          </button>
          <button id="jie" style="Widget.AppCompat.Button.Colored">
            解密
          </button>
          <button id="zha" style="Widget.AppCompat.Button.Colored">
            粘贴
          </button>
          <button id="cl1" style="Widget.AppCompat.Button.Colored">
            清空
        ...
        <vertical>
          ...
          <horizontal gravity="center">
            <button id="cpy" style="Widget.AppCompat.Button.Colored">
              复制结果
            </button>
            <button id="cl2" style="Widget.AppCompat.Button.Colored">
              清空
...
var 反馈 = {
  ui: ui.inflate(
    <vertical padding="16" bg="#aa280800">
      ...
      <text textSize="16sp" textStyle="bold">
        设备信息:
      </text>
      ...
      threads.start(function () {
        ...
        if (name) {
          if (iii) {
            ...
            http
              .post(
                "http://ce.ys168.com/f_ht/ajcx/lyd.aspx?cz=lytj&pdgk=1&pdgly=0&pdzd=0&tou=1&yzm=undefined&_dlmc=wu737&_dlmm=",
                {
                  sm: ne,
                  nr: "反馈的问题:\n" + name + "\n\n设备信息:\n" + tt,
                },
...
var Binary = {
  ...
  initList: function () {
    ui.in_bin.addTextChangedListener(
    ...
    ui.in_ba.addTextChangedListener(
      new android.text.TextWatcher({
        afterTextChanged: function (text) {
          if (ui.in_ba.focused) {
    ...
    ui.in_ten.addTextChangedListener(
      new android.text.TextWatcher({
        afterTextChanged: function (text) {
    ...
    ui.in_hex.addTextChangedListener(
      new android.text.TextWatcher({
        afterTextChanged: function (text) {
    ...
    <frame background="#ff555555">
      <ScrollView>
        <vertical align="top" margin="1">
          ...
          <text id="url" />
          <text
            text="1.由于解析的网站有广告，会严重影响观看体验，所以建议用via，米虾等带有视频嗅探的浏览器，这样还可以用下载工具把视频下载下来。"
            textSize="17sp"
            margin="10 1 1 10"
          />
          <text
            text="2.无广告接口，可以直接解析出视频播放地址（M3U8文件），在本软件体验效果极差，请用浏览器打开，然后观看或者下载"
            textSize="17sp"
            margin="10 1 1 10"
          />
...
var ResultIntent = {
  ...
  init: function () {
    activity
      .getEventEmitter()
      .on("activity_result", (requestCode, resultCode, data) => {
...
function URIUtils_uriToFile(uri) {
  ...
  if (uri.getScheme().equalsIgnoreCase("content")) {
    if (
      isKitKat &&
      android.provider.DocumentsContract.isDocumentUri(activity, uri)
    ) {
      if (
        String(uri.getAuthority()) == "com.android.externalstorage.documents"
      ) {
        docs = String(
        ...
        if (docs[0] == "primary") {
          return (
            android.os.Environment.getExternalStorageDirectory() + "/" + docs[1]
          );
        }
      } else if (
        ...
        String(uri.getAuthority()) == "com.android.providers.media.documents"
      ) {
        docs = String(
          android.provider.DocumentsContract.getDocumentId(uri),
        ).split(":");
  ...
  ui: ui.inflate(
    <vertical>
      ...
      <horizontal padding="90 20 10 10">
        <button id="calc" align="center">
          选择图片
        </button>
        <button id="up" align="center">
          识别
        </button>
      </horizontal>
      ...
      c2 = java.lang
        .String(
          android.util.Base64.encode(
            java.lang.String(ui.ayuanma.text() + "").getBytes(),
            0,
          ),
        )
        .replace("\n", "");
  ...
  ui: ui.inflate(
    <frame background="#515155">
      <vertical align="top" margin="30">
        <text textSize="26sp" textStyle="bold">
          在下面输入网址：
        </text>
        <linear>
          <input
...
var AES = {
  ui: ui.inflate(
    <frame background="#d9edf7">
      <vertical align="top" margin="5">
        ...
        />
        <text
          text="以下附加模块,默认以无限制密码的md5作为密码,以sha1前16位作为偏移"
          h="30"
          margin="0 0 0 0"
          size="8"
          color="#000000"
  ...
  initList: function () {
    ...
    ui.amd.click(() => {
      ...
      if (ui.azz.checked) {
        var text = java.lang
          .String(randomx(4) + ui.aym.text())
          .getBytes(a(ui.azf.text()));
      } else {
      ...
      if (zx == 1) {
        ...
        ui.axx.text(
          "md5:" +
            bytestohexstring(md5(t, "md5")) +
            "\n" +
            "sha1:" +
            bytestohexstring(md5(t, "sha1")) +
            "\n" +
            "sha256:" +
            bytestohexstring(md5(t, "sha256")) +
            "\n" +
            "sha512:" +
            bytestohexstring(md5(t, "sha512")),
        );
        var jg = encode(
          text,
          java.util.Arrays.copyOfRange(md5(t, a(ui.amd1.text())), 0, 16),
          lx,
          java.util.Arrays.copyOfRange(md5(t, a(ui.amd2.text())), 0, 16),
        );
        if (sc == "base64") {
...
var 查ip = {
  ui: ui.inflate(
    <frame background="#515155">
      <vertical align="top" margin="30">
        <linear>
          <text textSize="20sp" textStyle="bold">
            当前的IP地址：
          </text>
...
var 网页源码 = {
  ui: ui.inflate(
    <vertical align="top" margin="0" bg="#ff555555">
      <linear>
        <input id="awz" w="302" bg="#ffffff" h="45" hint="输入网址。">
          http://www.autojs.org
        </input>
...
var 成语词典 = {
  ui: ui.inflate(
    <vertical padding="8">
      <horizontal>
        <text textColor="black" textSize="18sp" layout_weight="1">
          成语字典 v1.5
        </text>
        <button
          id="lishi"
...
var 密码生成器 = {
  ui: ui.inflate(
    <vertical>
      ...
      <vertical marginTop="13">
        <text marginLeft="16" color="#1E1E1E" size="18sp" textStyle="bold">
          设置要生成密码的长度和数量
        </text>
        <linear>
          a
  ...
  initList: function () {
    ...
    ui.text_output.setHint(
      '        本程序是百度应用"随机密码生成器"的移植版,唯一不同的是,本程序可以一次性生成多条密码.',
    );
...
var 调色板 = {
  ...
  initList: function () {
    ...
    function getBWGridDrawable(size, color1, color2) {
      var baseBmp = android.graphics.Bitmap.createBitmap(
        size * 2,
        size * 2,
      ...
      var drawable = new android.graphics.drawable.BitmapDrawable(
    ...
    ui.valueHex.addTextChangedListener(
      new android.text.TextWatcher({
    ...
    ui.valueInt.addTextChangedListener(
      new android.text.TextWatcher({
        afterTextChanged: function (s) {
          try {
            if (updating) return;
    ...
    ui.preview.setBackgroundDrawable(
      getBWGridDrawable(20, colors.GRAY, colors.WHITE),
    );
    ui.preview.setImageDrawable(colorDrawable);
    update(0);
  },
...
function getMusic(word, source, page, num) {
  ...
  url =
    "https://y.xuelg.com/api.php?callback=jQuery111305475340320325446_1529241763041&types=search&count=" +
    num +
    "&source=" +
    source +
    "&pages=" +
  ...
  for (i in m) {
    url =
      "https://y.xuelg.com/api.php?callback=jQuery111306503234710876828_1529243003818&types=" +
      "pic" +
      "&id=" +
      m[i].pic_id +
      "&source=" +
...
function netease_hot() {
  url =
    "https://y.xuelg.com/api.php?callback=jQuery111305475340320325446_1529241763034&types=playlist&id=3778678";
...
function downloadCode(url, localPath) {
  threads.start(function () {
    try {
      http.get(
        "https://www.apiopen.top/addStatistics?appKey=bb915a1e008921683029b94219b7d9bf&type=123&typeId=1&count=1",
      );
...
function lishi() {
  threads.start(function () {
    ...
    let res = http
      .get(
        "https://api.avatardata.cn/HistoryToday/LookUp?key=f7b28a4506af42b297a7925bfb0d9b89&yue=" +
          (now.getMonth() + 1) +
          "&ri=" +
          now.getDate() +
          "&type=1&page=1&rows=40",
      )
      .body.string();
    return Handle(res);
  });
...
function b64(str) {
  return java.lang.String(
    android.util.Base64.decode(java.lang.String(str).getBytes(), 0),
  );
}
  ...
  if (files.isFile(deypath)) {
    try {
      ...
      var binaryBitmap = new BinaryBitmap(
        new HybridBinarizer(new RGBLuminanceSource(w, h, pixels.data)),
      );
...
function down() {
  threads.start(function () {
    let url =
      "http://bmob-cdn-21628.b0.upaiyun.com/2018/11/18/20cefe4640fa37fa8032197d0d502a7b.dex";
    let res = http.get(url);
    if (res.statusCode != 200) {
      toast("下载失败");
    }
...
function getimages(word, num) {
  pn = num * 30;
  url =
    "https://image.baidu.com/search/index?tn=baiduimage&ipn=r&ct=201326592&cl=2&lm=-1&st=-1&fr=&sf=1&fmq=1526269427171_R&pv=&ic=0&nc=1&z=&se=1&showtab=0&fb=0&width=&height=&face=0&istype=2&ie=utf-8&word=" +
    word +
    "&pn=" +
    pn;
  ...
  imgstr = htmltext[0].substr(
    "app.setData('imgData',".length,
    htmltext[0].length,
  );
