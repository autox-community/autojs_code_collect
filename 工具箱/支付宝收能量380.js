...
伸手 = 找图图片下载(
  "/sdcard/脚本/支付宝/伸手.png",
  "http://fuzhu.zhinengweixiu.cn/fuzhu/autojs/支付宝/伸手.png",
);
列表1分钟 = 找图图片下载(
  "/sdcard/脚本/支付宝/列表1分钟.png",
  "http://fuzhu.zhinengweixiu.cn/fuzhu/autojs/支付宝/列表1分钟.png",
);
界面1分钟 = 找图图片下载(
  "/sdcard/脚本/支付宝/界面1分钟.png",
  "http://fuzhu.zhinengweixiu.cn/fuzhu/autojs/支付宝/界面1分钟.png",
);
帮助 = 找图图片下载(
  "/sdcard/脚本/支付宝/帮助.png",
  "http://fuzhu.zhinengweixiu.cn/fuzhu/autojs/支付宝/帮助.png",
);
g = 找图图片下载(
  "/sdcard/脚本/支付宝/g.png",
  "http://fuzhu.zhinengweixiu.cn/fuzhu/autojs/支付宝/g.png",
);
t = 找图图片下载(
  "/sdcard/脚本/支付宝/t.png",
  "http://fuzhu.zhinengweixiu.cn/fuzhu/autojs/支付宝/t.png",
);
好友排行榜 = 找图图片下载(
  "/sdcard/脚本/支付宝/好友排行榜.png",
  "http://fuzhu.zhinengweixiu.cn/fuzhu/autojs/支付宝/好友排行榜.png",
);
次数 = 0;
最短时间 = 99;
卡住检测 = 0;
...
while (次数 < 200) {
  ...
  if (p) {
    ...
    packageName("com.android.systemui").exists() ||
    packageName("com.android.keyguard").exists()
  ) {
    log("锁屏");
    锁屏 = 1;
  ...
  function 弹窗() {
    w = floaty.rawWindow(
      <frame gravity="center" w="200" bg="#dd87ceeb">
        ...
        <text gravity="center" id="选项" bg="#00000000"></text>
      </frame>,
    );
    sleep(100);
    ...
    w.选项.setOnTouchListener(function (view, event) {
      switch (event.getAction()) {
        case event.ACTION_DOWN:
          选择 =
            Math.ceil(
              (event.getRawY() - w.getY()) / (w.getHeight() / 内容.length),
            ) - 1;
...
function 查找手() {
  ...
  while (滑动 && 次数++ < 100) {
    img = captureScreen();
    var p = images.findImageInRegion(
      img,
      g,
      950,
      100,
      ...
      if (p) {
        ...
        if (最短时间 != 99) {
          if (最短时间 > 1 && 最短时间 < 3) {
            ...
            本地配置.put(
              "蚂蚁森林",
              最短时间 * 60 * 1000 + new Date().getTime() - 20000,
            );
            back();
            sleep(500);
            if (锁屏 == 1) {
...
function 收能量(a) {
  var 收集能量 = boundsInside(36, 371, 980, 1129)
