var bilibiliCacheVideoPath = files.join(
  files.getSdcardPath(),
  "Android/data/tv.danmaku.bili/download/38379328",
);
r = files.exists(bilibiliCacheVideoPath);
log(r);
var arr = files.listDir(bilibiliCacheVideoPath);
log(arr);
arr.map((parentPathName) => {
...
function extractVideoInfo(parentPathName) {
  ...
  videoInfo.targetPath = files.join(
    bilibiliVideoSDPath,
    videoInfo.title + ".blv",
  );
  return videoInfo;
