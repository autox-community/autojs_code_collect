...
function 评论() {
  ...
  function QQ() {
    ...
    for (i = 0; i < number; i++) {
      //调用api获取评论
      var a = http
        .get("http://word.rainss.cn/api_system.php?type=txt")
        .body.string();
  ...
  function QQ空间() {
    app.launchPackage("com.tencent.mobileqq");
    toast("等待空间页面");
    waitForActivity("cooperation.qzone.QzoneFeedsPluginProxyActivity", [
      (period = 200),
    ]);
    for (i = 0; i < number; i++) {
      //调用api获取评论
  ...
  function 网易云音乐评论() {
    ...
    toast("打开音乐评论主页面");
    waitForActivity("com.netease.cloudmusic.activity.PlayerActivity", [
      (period = 200),
    ]);
    for (i = 0; i < number; i++) {
      //调用api获取评论
  ...
  function 微博评论() {
    ...
    toast("打开微博评论页面");
    waitForActivity("com.sina.weibo.feed.DetailWeiboActivity", [
      (period = 200),
    ]);
    for (i = 0; i < number; i++) {
      //调用api获取评论
...
function 联系作者() {
  toast(
    "本人精通CSS、JavaScript、PHP、ASP、C、C＋＋、C#、Java、Ruby、Perl、Lisp、python、Objective-C、ActionScript、Pascal等单词的拼写 ，及Windows、Linux、Andriod、OS X等环境的开关机。如有疑问请与我联系",
  );
  var qq号 = "2661621351";
  app.startActivity({
...
ui.four.on("click", () => {
  threads.start(function () {
    alert(
      "本人精通CSS、JavaScript、PHP、ASP、C、C＋＋、C#、Java、Ruby、Perl、Lisp、python、Objective-C、ActionScript、Pascal等单词的拼写 ，及Windows、Linux、Andriod、OS X等环境的开关机",
    );
  });
});
