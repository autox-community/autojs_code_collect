...
var 终点 = "东莞汽车公寓";
var 起点网页源码 = http.get(
  "https://ditu.amap.com/service/poiTipslite?&city=110000&type=dir&words=" +
    起点,
);
...
*/
var 表达式 =
  '{"tip":{"category":"[0-9]+","name":"([\\s\\S]*?)","district":"([\\s\\S]*?)","ignore_district":"([\\s\\S]*?)","adcode":"([\\s\\S]*?)","rank":"([\\s\\S]*?)","datatype_spec":"([\\s\\S]*?)","datatype":"([\\s\\S]*?)","x_entr":"([\\s\\S]*?)","address":"([\\s\\S]*?)","y":"([\\s\\S]*?)","x":"([\\s\\S]*?)","poiid":"([\\s\\S]*?)","y_entr":"([\\s\\S]*?)","id":"([\\s\\S]*?)","modxy":"([\\s\\S]*?)","lnglat":"([\\s\\S]*?)"}}'; //生成正则表达式
...
var str路径详情网页源码 = http.get(
  "https://ditu.amap.com/service/autoNavigat?usepoiquery=true&coor_need=true&rendertemplate=1&invoker=plan&engine_version=3&start_types=1&end_types=1&viapoint_types=1&policy2=2&fromX=" +
    str起点 +
    "&start_poiname=startpoint&toX=+" +
    str终点 +
    "&end_poiname=endpoint&viapoints=&viapoint_poiids=&viapoint_poinames=&viapoint_poitypes=&key=&callback=",
);
var str路径详情源码 = str路径详情网页源码.body.string(); //提取路途内容
//log(str路径详情源码)//调试输出专用，勿删，不用请注释

var str里程表达式 =
  'Successful.","distance":"([\\s\\S]*?),([\\s\\S]*?),([\\s\\S]*?)","';
...
var str时间表达式 =
  '","drivetime":"([\\s\\S]*?),([\\s\\S]*?),([\\s\\S]*?)","strategy';
