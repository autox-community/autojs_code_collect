...
var window = floaty.rawWindow(
  <vertical
    id="vertical"
    bg="#aaaaaa"
    w="{{Math.floor(device.width*0.9)}}px"
    h="{{Math.floor(device.width*0.9)}}px"
    gravity="center"
  >
...
threads.start(function () {
  sleep(100);
  window.setPosition(
    device.width / 2 - window.getWidth() / 2,
    device.height / 2 - window.getHeight() / 2,
  );
});
  ...
  threads.start(function () {
    ...
    if (MainImg) {
      ...
      if (isSaveToDir) {
        images.save(
          img,
          files.join(saveToDir, "/" + imgName + ".png"),
          "png",
          100,
        );
      }
...
window.canvas.post(function () {
  ...
  if (MainImg) {
    imageRect.set(
      new android.graphics.RectF(0, 0, MainImg.getWidth(), MainImg.getHeight()),
    );
    canvasMatrix.setRectToRect(
      imageRect,
      canvasRect,
...
window.canvas.on("draw", function (canvas) {
  ...
  if (MainImg) {
    ...
    canvas.drawText(
      "h: " + MainImg.getHeight(),
      MainImg.getWidth(),
      MainImg.getHeight() + TextSize,
      paint2,
    );
    //绘制区域
    canvas.drawRect(imageRect, paint);
    canvas.drawText(
      "w: " + Math.abs(imageRect.width()),
      imageRect.right,
      imageRect.bottom,
      paint2,
    );
    canvas.drawText(
      "h: " + Math.abs(imageRect.height()),
      imageRect.right,
      imageRect.bottom + TextSize,
      paint2,
    );
    canvas.drawText(
      "x: " + Math.abs(imageRect.left),
      imageRect.left,
      imageRect.top - TextSize,
      paint2,
    );
    canvas.drawText(
      "y: " + Math.abs(imageRect.top),
      imageRect.left,
      imageRect.top,
      paint2,
    );
...
window.canvas.setOnTouchListener(
  new android.view.View.OnTouchListener(function (view, event) {
    try {
      let W = view.getWidth();
...
function getTimeString() {
  return new java.text.SimpleDateFormat("yyyy_MM_dd_HH:mm:ss").format(
    new Date(),
  );
}
...
function 悬浮控制(window, windowid, ar) {
    ...
    if (!ar) {
      if (view) {
        ary = [
          [-view.getX(), -view.getY()],
          [
            this.Width - (view.getX() + view.getWidth()),
            this.Height - (view.getY() + view.getHeight()) - H - K - D,
          ],
        ];
      } else {
        ary = [
          [0, 0],
          [
            this.Width - win.getWidth() + K * 2,
            this.Height - win.getHeight() - H + K * 2 - D,
          ],
        ];
      }
    } else {
      if (view) {
        ary = [
          ...
          [
            this.Width - (view.getX() + view.getWidth()),
            this.Height - (view.getY() + view.getHeight()) - D,
          ],
        ];
      } else {
        ary = [
  ...
  this.OutScreen = () => {
    ...
    var cx =
        Math.abs(sx < this.Width - sx ? sx : this.Width - sx) <
        Math.abs(sy < this.Height - sy ? sy : this.Height - sy)
          ? sx < this.Width / 2
  ...
  this.ViewIntScreen = () => {
    //当悬浮超出屏幕之外之后进入的坐标。
    var A = this.windowGXY(
      window.getX(),
      window.getY(),
      this.G(window, windowid),
    );
    return [
      [window.getX(), window.getY()],
      [A.x, A.y],
