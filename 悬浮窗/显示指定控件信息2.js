...
var win_ = floaty.rawWindow(
  <frame
    ...
    h="{{Math.floor(device.width/2)}}px"
  >
    <button
      id="but"
      w="*"
    ...
    <button
      id="but_g"
      w="{{Math.floor(device.width/8)}}px"
      h="{{Math.floor(device.width/8)}}px"
      bg="#00000000"
      layout_gravity="center"
      gravity="center"
    />
  ...
  try {
    ...
    if (kg) {
      ...
      if (false) {
          ...
          if (uiObj) {
            ...
            if (i == 当前数组.length - 1) {
              ...
              for (let i = 0; i < show_ary.length; i++) {
                canvas.drawText(
                  show_ary[i] + ": " + uiObj[show_ary[i]](),
                  w / 10,
                  h / 5 + paint1.getTextSize() * i,
                  paint1,
                );
              }
            }
        ...
        paint1.setColor(
          rainbowColor[(当前数组.length - 1) % rainbowColor.length],
        );
