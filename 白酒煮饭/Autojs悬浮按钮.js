...
var win_1 = floaty.rawWindow(
  <frame id="logo" w="44" h="44" alpha="0.4">
    ...
    <img
      ...
      w="32"
      h="32"
      src="{{org.autojs.autojs.R.drawable.autojs_material}}"
      gravity="center"
      layout_gravity="center"
    />
...
var XY = [],
  ...
  dpB = 0,
  dp_H = 0;
events.broadcast.on("定时器关闭", function (X) {
  clearInterval(X);
});
...
var terid = setInterval(() => {
  ...
  if (TT.length == 0 && win.logo.getY() > 0) {
    // 不知道界面初始化的事件  只能放到这里将就下了
    ui.run(function () {
      (TT = [win.logo.getX(), win.logo.getY()]),
        (TT1 = [win.logo_1.getLeft(), win.logo_1.getTop()]),
        (anX = []),
        (anY = []); // 获取logo 绝对坐标
      XY = [
      ...
      XY1 = [
        [
          parseInt(dpZ * 41),
          TT1[0] - win.id_0.getLeft(),
          TT1[1] - win.id_0.getTop(),
        ],
        [
          parseInt(dpZ * -65),
          TT1[0] - win.id_1.getLeft(),
          TT1[1] - win.id_1.getTop(),
        ],
        [
          parseInt(dpZ * -106),
          TT1[0] - win.id_2.getLeft(),
          TT1[1] - win.id_2.getTop(),
        ],
        [
          parseInt(dpZ * -65),
          TT1[0] - win.id_3.getLeft(),
          TT1[1] - win.id_3.getTop(),
        ],
        [
          parseInt(dpZ * 41),
          TT1[0] - win.id_4.getLeft(),
          TT1[1] - win.id_4.getTop(),
        ],
      ];
                  ...
                  <frame w="*">
                    <frame marginLeft="20" w="auto">
                      <img
                        ...
                        w="26"
                        h="26"
                        src="@drawable/ic_accessible_black_48dp"
                      />
                    </frame>
                    <button
                      id="打开无障碍"
                  ...
                  <frame w="*">
                    <frame marginLeft="20" w="auto">
                      <img
                        ...
                        w="26"
                        h="26"
                        src="@drawable/ic_adb_black_48dp"
                        tint="#000000"
                      />
                    </frame>
                    <button
                  ...
                  <frame w="*">
                    <frame marginLeft="20" w="auto">
                      <img
                        ...
                        w="26"
                        h="26"
                        src="@drawable/ic_blur_on_black_48dp"
                        tint="#000000"
                      />
                    </frame>
                    <button
                  ...
                  <frame w="*">
                    <frame marginLeft="20" w="auto">
                      <img
                        ...
                        w="26"
                        h="26"
                        src="@drawable/ic_home_black_48dp"
                        tint="#000000"
                      />
                    </frame>
                    <button
  ...
  if (logo_switch) {
    if (logo_fx) {
      for (let i = 0; i < XY.length; i++) {
        anX[i] = ObjectAnimator.ofFloat(
          XY[i][0],
          "translationX",
          parseInt(XY[i][1]),
          0,
        );
        anY[i] = ObjectAnimator.ofFloat(
          XY[i][0],
          "translationY",
          parseInt(XY[i][2]),
          0,
...
function 对话框动画(X, Y, Z) {
  //X布尔值 标识显示还是隐藏 Y背景的视图 Z对话框的视图
  var anX = [],
    anY = [],
    slX = [],
    slY = [];
  if (X) {
    anX = ObjectAnimator.ofFloat(
      Z,
      "translationX",
      win_1.getX() - Z.getRight() / 2 + dpB - Z.getLeft(),
      0,
    );
    anY = ObjectAnimator.ofFloat(
      Z,
      "translationY",
      win_1.getY() - Z.getBottom() / 2 + img_dp.h_b - Z.getTop(),
      0,
    );
  ...
  set = new AnimatorSet();
  set.playTogether(anX, anY, slX, slY, animator, animator1);
  ...
  switch (event.getAction()) {
    ...
    case event.ACTION_MOVE:
      ...
      if (!yd) {
        ...
        //移动手指时调整两个悬浮窗位置
        win_1.setPosition(
          windowX + (event.getRawX() - x), //悬浮按钮定位
    ...
    case event.ACTION_UP: //手指弹起
      ...
      if (
        ...
        if (logo_switch) {
          ...
          win.setPosition(
            win_1.getX() + (event.getRawX() - x) - logo_right,
