...
flo.time.setOnTouchListener(function (v, e) {
  switch (e.getAction()) {
    ...
    case e.ACTION_MOVE:
      dxy = [
        Math.floor(e.getRawX() - xy0[0]),
        Math.floor(e.getRawY() - xy0[1]),
      ];
    ...
    switch (currentActivity()) {
      ...
      case "com.tencent.mm.ui.LauncherUI":
        ...
        if (sendButton != null) {
          ...
          if (
            !floatyState ||
            Math.abs(flo.getY() - bo.top + bo.height() * 1.3) > 2
          ) {
  ...
  try {
    ...
    threads.start(function () {
      switch (currentActivity()) {
        case "com.tencent.mobileqq.activity.SplashActivity":
        case "com.tencent.mobileqq.activity.ChatActivity":
          let sendButton = id("fun_btn")
            .text("发送")
            .enabled(true)
            .findOne(300);
          if (sendButton == null) return;
          let obj = className("android.widget.EditText")
            .editable(true)
            .findOne(300);
          if (obj == null) return;
        ...
        case "com.tencent.mm.ui.LauncherUI":
          sendButton = text("发送")
            ...
            .findOne(300);
          if (sendButton == null) return;
          obj = className("android.widget.EditText")
            .editable(true)
            .findOne(300);
          if (obj == null) return;
          objText = obj.text();
...
function runControl(stop) {
  ...
  for (i in arr) {
    if (
      arr[i].getSource().toString() == me.getSource().toString() &&
      arr[i] != me
    ) {
