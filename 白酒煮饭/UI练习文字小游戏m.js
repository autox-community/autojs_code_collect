...
function endlessTower(count) {
  ...
  ui.fight.click(() => {
    ...
    dialogs.singleChoice(str.action, skill2Arr(player.skill)).then((i) => {
...
function playerAction(skillid) {
  ...
  if (enemy.hp > finalDanger) enemy.hp -= finalDanger;
  else enemy.hp = 0;
  return {
    type: skillid,
    danger: finalDanger,
...
function enemyAction() {
  ...
  finalDanger = finalDanger <= 0 ? 1 : finalDanger;
  if (player.hp > finalDanger) player.hp -= finalDanger;
  else player.hp = 0;
  return {
    type: skillid,
    danger: finalDanger,
...
function equipForge() {
  ...
  ui.list.on("item_bind", function (itemView, itemHolder) {
    itemView.list.on("click", function () {
      alertDialog(
        str[itemHolder.item.type] + str.forge + str.info,
        str.forge + str.lv + itemHolder.item.val,
      );
    });
  ...
  ui.item_list.on("item_bind", function (itemView, itemHolder) {
    itemView.item_list.on("click", function () {
      ...
      alertDialogE(it.name, msg4Gdata2Str(gdata[it.type][it.id]), positive)
        .on("positive", () => {
          if (it.type == types.item) useItem(it.id);
          else if (equips.indexOf(it.type) > -1) changeEquip(it.type, it.id);
          personBag(type);
        })
        .show();
    });
...
function changeEquip(type, id) {
  player.equip.forEach(function (equip) {
    if (equip.type == type) {
      if (equip.id > 0) gain(type, equip.id, 1);
      lost(type, id, 1);
      equip.id = id;
...
function gridLayout(id, span) {
  return (
    <grid id={id} spanCount={span} w="auto" h="auto" align="center">
...
function showBackBtn() {
  return (
    <fab
      ...
      h="auto"
      tint={COLOR_GREEN}
      src="@drawable/ic_subdirectory_arrow_right_black_48dp"
      text={str.back}
      align="bottom|right"
    />
  );
...
function title(text) {
  return (
    <linear w="*" marginButton="15" pading="15" bg={COLOR_GREEN}>
...
function showBagMenuByTypes(type) {
  ...
  Object.keys(types).forEach(function (key) {
    let t = types[key];
    let c =
      t == type ? "color='" + COLOR_WHITE + "' bg='" + COLOR_GREEN + "'" : "";
    xml +=
      "<text id='" +
      t +
      "' text='" +
      str[t] +
      "' w='auto' padding='10 5 10 5' " +
      c +
      "/>";
  });
