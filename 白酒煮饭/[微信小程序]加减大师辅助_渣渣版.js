...
while (true) {
  ...
  var img_Question_Base64 = images.toBase64(
    img_Question,
    (format = "png"),
    (quality = 100),
  );
  var img_Answer_Base64 = images.toBase64(
    img_Answer,
    (format = "png"),
    (quality = 100),
  );
  ...
  try {
    ...
    log(operation);
    if (operation.words_result_num == 0) {
      throw "err";
    }
    if (operation.error_code != undefined) {
      throw "err1";
    }
    ...
    log(result_str);
    if (result_str.words_result_num == 0) {
      throw "err";
    }
    if (result_str.error_code != undefined) {
      throw "err1";
    }
    result_str = result_str.words_result[0].words;
    var result = result_str.replace(/[^-|^0-9]/gi, "");
  } catch (e) {
    sleep(200);
    continue;
  }
