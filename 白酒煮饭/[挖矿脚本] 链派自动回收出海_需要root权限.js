...
pref
  .edit()
  .putBoolean("key_enable_accessibility_service_by_root", true)
  .commit();
...
// while (true) {
var Info = http.post(
  "http://newpai.xiaoxi6.com/lianpai/navigation/button_list.html",
  {
    user_id: userId,
  },
  getHeaders(),
...
for (let i = 0; i < fleetListArry.length; i++) {
  ...
  if (status == 2) {
    var outShip = http.post(
      "http://newpai.xiaoxi6.com/lianpai/navigation/out_ship.html",
      {
        user_id: userId,
        ship_id: shipId,
      },
  ...
  if (status == 4) {
    ...
    if (needShare(userId, shipId)) {
      ...
      if (currentActivity() != "com.xi6666.lpsq.ui.web.Html5Activity") {
        app.startActivity({
          ...
          extras: {
            url:
              "http://newpai.xiaoxi6.com//lianpai/navigation/index?_ver=2.1.0&_dtype=Android&_t=" +
              TimeStamp +
              "&_utoken=" +
              utoken +
              "&user_id=" +
    ...
    while (needShare(userId, shipId)) {
      sleep(500);
    }

    var charge = http.post(
      "http://newpai.xiaoxi6.com/lianpai/navigation/charge.html",
      {
        user_id: userId,
        ship_id: shipId,
      },
...
function needShare(userId, shipId) {
  var updateShare = http.post(
    "http://newpai.xiaoxi6.com/lianpai/navigation/updateshare.html",
    {
      user_id: userId,
      ship_id: shipId,
    },
...
function getHeaders() {
  ...
  var header = {
    headers: {
      ...
      Referer:
        "http://newpai.xiaoxi6.com//lianpai/navigation/index?_ver=2.2.0&_dtype=Android&_t=" +
        TimeStamp +
        "&_utoken=" +
        utoken +
        "&user_id=" +
