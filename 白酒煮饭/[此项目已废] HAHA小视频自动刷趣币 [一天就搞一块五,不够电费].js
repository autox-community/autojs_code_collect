        ...
        if (newTime - oldTime > 1000) {
          ...
          if (newTitleNum != oldTitleNum) {
            ...
            if (device.sdkInt < 24) {
              ...
              ra.swipe(x0, y0, device.width * 0.7, device.height * 0.15, 400);
              sleep(500);
            } else {
              var points = [500];
              var interval = 0.1;
              ...
              for (var t = 0; t < Math.PI / 2; t += interval) {
                var x =
                  x0 - a * (1.8 * Math.cos(t * 0.9) - Math.cos(2 * t * 0.9));
                var y =
                  y0 - a * (6 * Math.sin(t * 0.9) - Math.sin(3 * t * 0.9));
...
function clickId(a) {
  for (
    obj_ID = id(a).boundsInside(0, 5, device.width, device.height - 5);
    obj_ID.find().empty();
