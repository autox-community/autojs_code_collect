...
while (true) {
  ...
  var redbagList = http.post(
    "http://bdhb.shuangpinkeji.com/api/m1/redbag/redbag-list",
    {
      longitude: point.x,
      latitude: point.y,
      uid: uid,
  ...
  if (list.length > 0) {
    for (let i = 0; i < list.length; i++) {
      ...
      var click = http.post(
        "http://bdhb.shuangpinkeji.com/api/m1/redbag/click",
        {
          type: "2",
          longitude: long,
          latitude: lat,
      ...
      var sysReceive = http.post(
        "http://bdhb.shuangpinkeji.com/api/m1/redbag/sys-receive",
        {
          uid: uid,
          sign: sign,
          id: id,
  ...
  var header = {
    headers: {
      ...
      "User-Agent":
        ...
        " Build/" +
        build +
        "; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/66.0.3359.126 Mobile Safari/537.36 Html5Plus/1.0",
