...
function canvasDraw(canvasView) {
  this.distance = 60;
  this.kg = false;
  this.Color = colors.BLACK; //黑色。
  this.rectsAry = new Array();
  ...
  this.drawRects = function (canvas, ary, matrix, paint) {
    canvas.save();
    canvas.setMatrix(matrix);
    for (let i = 0; i < ary.length; i++) {
      paint.setColor(ary[i].color);
      canvas.drawRect(ary[i].rect, paint);
    }
    canvas.restore();
  };

  this.resetPoints = function () {
    this.rectsAry = this.pointsToRect(
      this.rectPoints(
        ...
        switch (event.getActionMasked()) {
          case event.ACTION_MOVE:
            for (let i = 0; i < PC; i++) {
              ...
              this.Segmenting(
                [
                  this.TouchPointRecord[i][0],
                  this.TouchPointRecord[i][1],
                  X,
                  Y,
                ],
                (X, Y) => {
                  this.main2([X, Y], this.rectsAry);
                },
              );
              this.TouchPointRecord[i].unshift(X, Y);
            }
          ...
          default:
            ...
            switch (event.getActionMasked()) {
              case event.ACTION_DOWN:
                ...
                this.TouchPointRecord.splice(I, 0, [X, Y]);
                this.main2([X, Y], this.rectsAry);
                break;
