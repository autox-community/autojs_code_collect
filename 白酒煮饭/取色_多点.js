...
log = function () {
  ...
  ui.run(() => {
    ui.ConsoleText.setText(
      new Date().toTimeString().split(" ")[0] + "/: " + String(arg),
    );
  });
};
...
function 图片查看(canvasView, img) {
  ...
  this.resetCanvasViewData = function () {
    ...
    this.canvasRect = new android.graphics.RectF(
      0,
      0,
      this.view.w,
      this.view.h,
  ...
  this.resetImgData = function () {
    this.imgRect = new android.graphics.RectF(
      0,
      0,
      img.getWidth(),
      img.getHeight(),
  ...
  this.resetPointsData = function () {
    this.pointsRect = new android.graphics.RectF(
      -this.view.w / 2,
      -this.view.h / 2,
      this.view.w / 2,
      this.view.h / 2,
    );
    this.pointsMatrix.setRectToRect(
      this.pointsRect,
      this.canvasRect,
  ...
  this.drawRect = function (canvas, rect, paint, Matrix) {
    let X = rect.left,
      ...
      x = rect.right,
      y = rect.bottom;
    let ary = this.matrixPoints(Matrix, [
      X,
      Y,
      x,
      Y,
  ...
  this.drawCircle = function (canvas, rect, paint, Matrix) {
    ...
    canvas.drawCircle(
      ary[0],
      ary[1],
      (this.weiyi([ary[2] - ary[4], ary[3] - ary[5]]) +
        this.weiyi([ary[2] - ary[6], ary[3] - ary[7]])) /
        4,
      paint,
    );
  };
  ...
  this.resetPoints = function () {
    this.Points.rect = this.rectPoints(
      this.pointsRect.width(),
      this.pointsRect.height(),
      this.PointSpacing(
        this.pointsRect.width(),
        this.pointsRect.height(),
        this.maxColorsPoints,
      ),
    );
    this.Points.strokeRect = this.rectPoints(
      this.pointsRect.width(),
      this.pointsRect.height(),
      this.PointSpacing(
        this.pointsRect.width(),
    ...
    this.Points.circle = this.circlePoints(
      (this.pointsRect.width() + this.pointsRect.height()) / 4,
      this.PointSpacing(
        this.pointsRect.width(),
        this.pointsRect.height(),
        this.maxColorsPoints,
    ...
    this.Points.strokeCircle = this.circlePoints(
      (this.pointsRect.width() + this.pointsRect.height()) / 4,
      this.PointSpacing(
        this.pointsRect.width(),
        this.pointsRect.height(),
  ...
  this.countPoints = function (ary) {
    ...
    return this.matrixPoints(
      this.imgInvertMatrix,
      this.matrixPoints(this.pointsMatrix, ary),
    );
  };
  ...
  this.mainCount = function () {
    try {
      ...
      //获取屏幕坐标点组
      this.PointsAry.XYAry = this.toColorsAry(
        ary2,
        this.imgInvertMatrix,
        img,
        this.isColor,
  ...
  this.Loading = function (canvas, paint, RY) {
    ...
    canvas.drawText(
      "侧拉菜单中打开一个图片",
      w / 2,
      h * 0.48 + 0.365 * paint.getTextSize(),
      paint,
    );
  };
  ...
  ui.run(() => {
    ...
    var explorerView = new ExplorerView(
      new android.view.ContextThemeWrapper(
        context,
        org.autojs.autojs.R.style.AppTheme,
      ),
    );
    explorerView.setExplorer(
      Explorers.workspace(),
      ExplorerDirPage.createRoot("/sdcard"),
    );
    explorerView.setDirectorySpanSize(2);
    var dialog =
      new org.autojs.autojs.theme.dialog.ThemeColorMaterialDialogBuilder(
        context,
      )
        .title("选择图片文件")
      ...
      <vertical>
        ...
        <list id="list" w="*">
          <horizontal
            w="*"
            margin="5"
            bg={colors.toString(colors.GRAY)}
            gravity="center"
          >
            <img
              w="{{Math.round(device.width/7)}}px"
              h="{{Math.round(device.width/7)}}px"
              margin="6"
              scaleType="fitXY"
              src="file://{{filePath}}"
            />
...
function XmlToView(xml) {
  ...
  return runtime.ui.layoutInflater.inflate(
    xml.toXMLString().toString(),
    null,
    true,
  );
}
...
function getAudiosInfo(maxAmount, ary) {
  ...
  while (cursor.moveToPrevious() && Ary.length < maxAmount) {
    let ob = {};
    ob._id = cursor.getString(
      cursor.getColumnIndexOrThrow(MediaStore.Audio.Media._ID),
    );
    ob.filePath = cursor.getString(
      cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA),
    );
    ob.title = cursor.getString(
      cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.TITLE),
    );
    ob.mime_type = cursor.getString(
