...
threads.start(function () {
  ...
  while (true) {
    ...
    paint.setStrokeWidth(10); //边缘宽度
    canvas.drawLine(
      100 * Math.cos(针尾度数) + 540,
      100 * Math.sin(针尾度数) + 540,
      200 * Math.cos(度数) + 540,
      200 * Math.sin(度数) + 540,
      paint,
    ); //绘制直线
    ...
    paint.setStrokeWidth(10); //边缘宽度
    canvas.drawLine(
      90 * Math.cos(针尾度数) + 540,
      90 * Math.sin(针尾度数) + 540,
      300 * Math.cos(度数) + 540,
      300 * Math.sin(度数) + 540,
      paint,
    ); //绘制直线
    ...
    if (存图否 == 1) {
      var mFile = new java.io.File(
        sdpath + "/Screenshot/Screenshot_" + new Date().getTime() + ".png",
      );
...
function getSDPath() {
  sdDir = null;
  sdCardExist = android.os.Environment.getExternalStorageState().equals(
