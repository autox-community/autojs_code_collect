...
function coordinateAnalysisAndExtract(img) {
  ...
  for (var i = 1; i <= HorizontalNumber - 1; i++) {
    for (var j = 1; j <= VerticalNumber - 1; j++) {
      ...
      var ColorMainColorBlock = images.pixel(
        img,
        xStart + (i - 1) * SpacingLeftAndRight,
        yStart + (j - 1) * SpacingUpAndDown,
      );
      ...
      var ColorRightColorBlock = images.pixel(
        img,
        xStart + i * SpacingLeftAndRight,
        yStart + (j - 1) * SpacingUpAndDown,
      );
      var ColorDownColorBlock = images.pixel(
        img,
        xStart + (i - 1) * SpacingLeftAndRight,
        yStart + j * SpacingUpAndDown,
      );
      ...
      if (
        colors.isSimilar(
          ColorMainColorBlock,
          ColorRightColorBlock,
          threshold,
        ) ||
        colors.isSimilar(ColorMainColorBlock, ColorDownColorBlock, threshold)
      ) {
...
function coordinateAnalysisAndExtractDown(img) {
  ...
  for (var i = 1; i <= HorizontalNumber - 1; i++) {
    ...
    threshold = Threshold;
    if (
      colors.isSimilar(ColorMainColorBlock, ColorRightColorBlock, threshold)
    ) {
      var targetCoordinates = new Array(
        xStart + (i - 1) * SpacingLeftAndRight,
        yStart,
