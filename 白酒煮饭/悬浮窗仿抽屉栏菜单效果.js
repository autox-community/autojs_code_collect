  ...
  timer = setInterval(function () {
    ...
    if (alpha <= 255 && alpha >= 0) {
      parentLayout.setBackgroundColor(
        android.graphics.Color.argb(alpha, 22, 22, 22),
      );
    }
    ...
    if (view.x == target) {
      if (isShow) {
        parentLayout.setBackgroundColor(
          android.graphics.Color.argb(144, 22, 22, 22),
        );
      } else {
        parentLayout.setBackgroundColor(
          android.graphics.Color.argb(0, 22, 22, 22),
        );
...
function setMarginX(view, left) {
  var margin = new android.view.ViewGroup.MarginLayoutParams(
