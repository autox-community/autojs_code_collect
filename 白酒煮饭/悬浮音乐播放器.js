...
function bianju() {
  ...
  A.push(
    daxiao[DX][0] / 2 - 35,
    daxiao[DX][1] / 2 - 35,
    pingwidth - (daxiao[DX][0] / 2 - 35),
    pingheight - (daxiao[DX][1] / 2 - 35) - 70,
  );
  //log(A)
  return A;
}
...
function setontouch() {
  ...
  for (var i = 0; i < A.length; i++) {
    K.push(
      " window.but" +
        A[i] +
        ".setOnTouchListener(function(view, event){switch(event.getAction()){case event.ACTION_DOWN: butkg" +
        A[i] +
        "=true;return true;case event.ACTION_MOVE:return true;case event.ACTION_UP: butkg" +
        A[i] +
    ...
    if (files.isDir(dirs[i])) {
      var File = files.listDir(dirs[i], function (name) {
        return (
          ...
          files.isFile(files.join(dirs[i], name))
        );
      });
      if (File.length > 0) {
...
var ax,
  ...
  fangxiang = true;
sensors
  .register("gravity", sensors.delay.ui)
...
sleep(100);
window.setPosition(
  (pingwidth - window.getWidth()) / 2 + 1,
  (pingheight - window.getHeight()) / 2,
);
var DX = 0,
  daxiao = [
    [960, 720],
...
var dir = ["/sdcard/qqmusic/song", "/sdcard/kgmusic/download"];
...
ui.run(function () {
  ...
  A = A.toString();
  var B = Math.round(
    (device.getMusicVolume() / device.getMusicMaxVolume()) * 100,
  );
  B = B.toString();
  ...
  switch (event.getAction()) {
    ...
    case event.ACTION_UP:
      ...
      if (yidong) {
        ...
        var xy = windowGXY(
          gx + window.getWidth() / 2,
          gy + window.getHeight() / 2,
          bianju(),
        );
        window.setPosition(
          xy[0] - window.getWidth() / 2,
          xy[1] - window.getHeight() / 2,
        );
        yidong = false;
      }
      return true;
...
function runTick() {
  if (kg) {
    var jindu =
      Math.round(
        (media.getMusicCurrentPosition() / media.getMusicDuration()) * 1000,
      ) / 10;
  ...
  if (butkgB) {
    device.setMusicVolume(
      device.getMusicVolume() - device.getMusicMaxVolume() / 20,
    );
  }
  if (butkgD) {
    device.setMusicVolume(
...
function tiaohuan() {
  ...
  if (DX == 0) {
    ...
    if (x <= pingwidth / 2) {
      yidongtexiao(
        [
          pingwidth / 2 + daxiao[0][0] / 2 - daxiao[1][0],
          window.getY(),
          x_y[0] - daxiao[DX][0] / 2,
          window.getY(),
        ],
    ...
    if (x <= pingwidth / 2) {
      yidongtexiao(
        [
          xay[0] - daxiao[DX][0] / 2 - daxiao[DX][0],
          xay[1] - daxiao[DX][1] / 2,
          xay[0] - daxiao[DX][0] / 2,
          xay[1] - daxiao[DX][1] / 2,
        ],
