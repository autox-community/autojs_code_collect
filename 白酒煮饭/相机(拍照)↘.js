...
var SaveDirPath = FGH.Environment.getExternalStoragePublicDirectory(
  FGH.Environment.DIRECTORY_PICTURES,
);
log(SaveDirPath);
/*
注意：
...
function checkCameraHardware(context) {
  if (
    context
      .getPackageManager()
      .hasSystemFeature(FGH.PackageManager.FEATURE_CAMERA)
  ) {
...
mCamera.setOneShotPreviewCallback(
  new FGH.Camera.PreviewCallback({
...
var mAutoFocusCallback = new FGH.Camera.AutoFocusCallback({
  onAutoFocus: function (success, camera) {
    ...
    if (success) {
      ...
      //这个有拍照声音。
      camera.takePicture(
        new FGH.Camera.ShutterCallback({
          //onShutter: function() {
          //    },
        }),
        null,
...
ui.surface.setOnClickListener(
  new FGH.View.OnClickListener({
    onClick: function (view) {
      try {
        //自动聚焦。
        mCamera.autoFocus(
          new FGH.Camera.AutoFocusCallback({
            onAutoFocus: function (success, camera) {
              if (success) {
                log("聚焦成功");
              } else {
