//console.show();
//悬浮窗样式
xa = b64(
  "",
);
xb = b64(
  "",
);
//弹框样式
xdjs = b64(
  "",
);
a = b64(
  "",
);
b = b64(
  "",
);
path = "/sdcard/脚本/";
pa2 = "/sdcard/脚本/打包后脚本.js";
var jsFiles = files.listDir(path, function (name) {
  return name.endsWith(".js") && files.isFile(files.join(path, name));
});
jsFiles = jsFiles.sort(function compareFunction(param1, param2) {
  return param1.localeCompare(param2, "zh");
}); //按拼音排序

var nr = new Array();
var nm = new Array();
for (i in jsFiles) {
  nr[i] = ec(files.read(path + jsFiles[i]));
  nm[i] = '"' + jsFiles[i].split(".js")[0] + '"';
}
t1 = "var jb=[" + nm.join(",") + "];";
t2 = "var nr=[\n" + nr.join(",\n") + "\n];";
files.write(pa2, xa + xdjs + "\n" + t1 + "\n" + t2 + xb);
toast("ok");

function ec(str) {
  return (
    'b64("' +
    java.lang
      .String(android.util.Base64.encode(java.lang.String(str).getBytes(), 0))
      .replace("\n", "") +
    '")'
  );
}
function b64(str) {
  return java.lang.String(
    android.util.Base64.decode(java.lang.String(str).getBytes(), 0),
  );
}
