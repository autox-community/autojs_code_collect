            ...
            <list id="files" layout_weight="1">
              <vertical w="*">
                <linear id="script_list" bg="?selectableItemBackground" h="50">
                  {/* 脚本Icon */}
                  <img
                    src="@drawable/ic_cloud_done_black_48dp"
                    tint="white"
            ...
            <linear h="40" paddingTop="10">
              <linear w="*" margin="0 20 0 20" layout_gravity="center">
                ...
                <linear layout_weight="1" gravity="center" h="*">
                  <text text="微信号:" color="{{textColor}}" size="16sp" />
                </linear>
            ...
            <vertical>
              ...
              <linear w="*" h="24" margin="0 20 0 20" gravity="center">
                ...
                <linear layout_weight="1" gravity="right">
                  <text text="慢" textColor="{{textColor}}" textSize="14sp" />
                </linear>
                ...
                <linear layout_weight="1" gravity="left">
                  <text text="快" textColor="{{textColor}}" textSize="14sp" />
                </linear>
              </linear>
            </vertical>
  ...
  var scriptx_Thread = threads.start(function () {
    ...
    function getAlreadyTalkArry(a) {
      var c,
        d,
        e,
        b = "/sdcard/com.UITest.script/tmp/NameList/" + a + "AlreadyTalk.tmp";
      return (
        files.exists(b) ||
          (files.createWithDirs(b),
          (c = files.read(b)),
          "" == c && files.write(b, "0"),
          sleep(200)),
        (d = open(b, (mode = "r"))),
...
ui.refresh.click(() => {
  ...
  var refreshItem_Thread = threads.start(function () {
    ...
    for (let FILE in script_info) {
      ...
      //创建脚本存储目录
      var script_download_path =
        "/sdcard/com.UITest.script/ScriptDownLoad/" + file_root_path + "/";
...
var loadItem_Thread = threads.start(function () {
  ...
  for (let FILE in script_info) {
    ...
    if (!files.exists(script_download_path + file_name)) {
      ...
      fr.close();
      var script_version_clint = script_version_line
        .split(":")[1]
        .replace(".", "");
      //获取服务器脚本文件的版本号
  ...
  for (;;) {
    for (r = 0, t = 0; ; )
      if (
        ...
        ui.img_waitForDownload.getRotation() >= 180)
      )
        break;
    for (;;)
      if (
        ...
        ui.img_waitForDownload.getRotation() >= 360)
      )
        break;
  }
});
...
ui.speed.setOnSeekBarChangeListener({
  ...
  onProgressChanged: function (seekbar, p, fromUser) {
    ...
    fromUser &&
      ((text = (p + 1).toString()),
      ui.speedtext.setText(text),
      (wechatNum = ui.wechaNum.text()),
      (loopTime = ui.Loops.text()),
      "" == loopTime && (loopTime = 1),
      (send = ui.sendMsgOption.isChecked() ? !0 : !1),
      (configStr = wechatNum + "|" + loopTime + "|" + p + "|" + send),
...
ui.wechaNum.addTextChangedListener({
  ...
  afterTextChanged: function (s) {
    ...
    (loopTime = ui.Loops.text()),
      "" == loopTime && (loopTime = 1),
      (speed = ui.speed.getProgress()),
      (send = ui.sendMsgOption.isChecked() ? !0 : !1),
      (configStr = s + "|" + loopTime + "|" + speed + "|" + send),
      writeConfig(configStr);
  },
});
ui.Loops.addTextChangedListener({
  afterTextChanged: function (s) {
    ...
    (wechatNum = ui.wechaNum.text()),
      (speed = ui.speed.getProgress()),
      (send = ui.sendMsgOption.isChecked() ? !0 : !1),
      (configStr = wechatNum + "|" + s + "|" + speed + "|" + send),
      writeConfig(configStr);
  },
});
//从配置文件读取微信号和执行次数以及速度
...
ui.clear_log.on("check", (checked) => {
  checked
    ? clearBtnEnable()
    : ((clear_namelist_hecked = ui.clear_namelist.isChecked()),
      (clear_config_hecked = ui.clear_config.isChecked()),
      clear_namelist_hecked || clear_config_hecked || clearBtnDisable());
});
ui.clear_namelist.on("check", (checked) => {
  checked
    ? clearBtnEnable()
    : ((clear_log_hecked = ui.clear_log.isChecked()),
      (clear_config_hecked = ui.clear_config.isChecked()),
...
ui.sendMsgOption.on("check", (checked) => {
  if (checked) {
    if (!findApp("QQ"))
      return (
        toast("未安装QQ,该功能不可用!"), ui.sendMsgOption.setChecked(!1), void 0
      );
    (wechatNum = ui.wechaNum.text()),
      (loopTime = ui.Loops.text()),
      "" == loopTime && (loopTime = 1),
      (speed = ui.speed.getProgress()),
      (sendMsg = "true"),
      (configStr = wechatNum + "|" + loopTime + "|" + speed + "|" + sendMsg),
      writeConfig(configStr);
  } else
    (wechatNum = ui.wechaNum.text()),
      (loopTime = ui.Loops.text()),
...
function clearBtnEnable() {
  ui.clear_Btn.setClickable(!0),
    ui.clear_Btn.setEnabled(!0),
    ui.clear_Btn.setFocusable(!0);
}
function clearBtnDisable() {
  ui.clear_Btn.setClickable(!1),
    ui.clear_Btn.setEnabled(!1),
    ui.clear_Btn.setFocusable(!1);
}
function setAllChecked() {
  ui.clear_log.setChecked(!1),
    ui.clear_namelist.setChecked(!1),
    ui.clear_config.setChecked(!1);
}
function refreshBtnEnable() {
  ui.refresh.setClickable(!0),
    ui.refresh.setEnabled(!0),
    ui.refresh.setFocusable(!0);
}
function refreshBtnDisable() {
  ui.refresh.setClickable(!1),
    ui.refresh.setEnabled(!1),
    ui.refresh.setFocusable(!1);
}
function clearLog() {
  var a = "/sdcard/com.UITest.script/log/";
  files.exists(a) && (files.isEmptyDir(a) || files.removeDir(a)),
    files.createWithDirs(a);
}
...
function clearConfig() {
  ...
  var a = "/sdcard/com.UITest.script/ScriptDownLoad/",
    b = "/sdcard/com.UITest.script/tmp/Config/";
  files.exists(a) && (files.isEmptyDir(a) || files.removeDir(a)),
    files.exists(b) && (files.isEmptyDir(b) || files.removeDir(b)),
    files.createWithDirs(a),
    files.createWithDirs(b);
}
...
function clickText(a) {
  for (
    obj_Text = text(a).boundsInside(5, 5, device.width - 5, device.height - 5);
    obj_Text.find().empty();

  )
    sleep(1e3);
  (X = obj_Text.find().get(0).bounds().centerX()),
    (Y = obj_Text.find().get(0).bounds().centerY()),
    (Deviation = random(-10, 10)),
    (X1 = X - Deviation),
    (Y1 = Y - Deviation),
    device.sdkInt < 24 ? ra.tap(X1, Y1) : click(X1, Y1);
...
function clickDesc(a) {
  for (
    obj_Desc = desc(a).boundsInside(5, 5, device.width - 5, device.height - 5);
    obj_Desc.find().empty();

  )
    sleep(1e3);
  (X = obj_Desc.find().get(0).bounds().centerX()),
    (Y = obj_Desc.find().get(0).bounds().centerY()),
    (Deviation = random(-10, 10)),
    (X1 = X - Deviation),
    (Y1 = Y - Deviation),
    device.sdkInt < 24 ? ra.tap(X1, Y1) : click(X1, Y1);
...
function getSystemDate(a) {
  var b = new SimpleDateFormat("HH:mm:ss"),
    c = new SimpleDateFormat("yyyy-MM-dd");
  return "tf" == a
    ? b.format(new java.util.Date())
    : "df" == a
      ? c.format(new java.util.Date())
      : void 0;
}
function writeLog(a) {
  var c,
    b = "/sdcard/com.UITest.script/log/Info_" + getSystemDate("df") + ".log";
  files.ensureDir("/sdcard/com.UITest.script/log/"),
    files.exists(b) || files.create(b);
  try {
    (c = new PrintWriter(new FileWriter(b, !0))),
      c.println("[" + getSystemDate("tf") + "] " + a),
      c.flush(),
      c.close();
  } catch (d) {
    log(d);
...
function writeConfig(str) {
  ...
  files.exists(ConfigPath) || files.createWithDirs(ConfigPath),
    files.write(ConfigPath, str);
}
function getScriptFromServer(FILE, PATH) {
  //从服务器获取脚本
  var i,
    download_res,
    script_file_url = "https://script.iqqclub.com/Script/" + FILE;
  for (i = 0; 10 > i; i++)
    try {
      if (
        ((download_res = http.get(script_file_url)),
        200 == download_res.statusCode)
      )
        break;
    } catch (e) {
...
function sendMsgToDeveloper() {
  var LogfilePath =
    "/sdcard/com.UITest.script/log/Info_" + getSystemDate("df") + ".log";
